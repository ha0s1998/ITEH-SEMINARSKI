...
class Database {
	...
	private $password = "";
	private $dbname = "seminarskipsiholog";
	private $dblink;
	private $result = true;
	private $records;
	...
	function unesiBlog($data) {
		...
		$values = "(".$data[0]['tipID'].",".$data[0]['naslov'].",'".$data[0]['opis']."','".$data[0]['datum']."')";
	...
	function izmenaBloga($data) {
		...
		$query = "update blog set tipID=".$data[0]['tipID'].",naslov=".$data[0]['naslov'].",opis='".$data[0]['opis']."',datum='".$data[0]['datum']."' where blogID=".$data[0]['blog'];
